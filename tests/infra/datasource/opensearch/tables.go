...
var tables = map[string]*test_utils.Table[string, *array.StringBuilder]{
	...
	"pushdown_null": {
		...
		Schema: &test_utils.TableSchema{
			Columns: map[string]*Ydb.Type{
				...
				"string":   common.MakeOptionalType(common.MakePrimitiveType(Ydb.Type_STRING)),
				"objectid": common.MakeOptionalType(common.MakePrimitiveType(Ydb.Type_STRING)),
			},
		},
		Records: []*test_utils.Record[[]byte, *array.BinaryBuilder]{{
			Columns: map[string]any{
	...
	"pushdown_not_null": {
		...
		Records: []*test_utils.Record[[]byte, *array.BinaryBuilder]{{
			Columns: map[string]any{
				...
				"string":  []*[]byte{ptr.Bytes([]byte("exists")), ptr.Bytes([]byte("exists2"))},
				"objectid": []*[]byte{
					ptr.Bytes([]byte("507f1f77bcf86cd799439011")),
					ptr.Bytes([]byte("507f1f77bcf86cd799439012")),
				},
			},
		}},
	},
	"pushdown_comparisons_eq": {
		...
		Records: []*test_utils.Record[[]byte, *array.BinaryBuilder]{{
			Columns: map[string]any{
				...
				"string":  []*[]byte{ptr.Bytes([]byte("outer"))},
	...
	"strcomp": {
		...
		Records: []*test_utils.Record[[]byte, *array.BinaryBuilder]{{
			Columns: map[string]any{
				...
				"a":   []*[]byte{ptr.Bytes([]byte("abc def")), ptr.Bytes([]byte("def abc"))},
			},
		}},
	},
	"columns_l": {
	...
	"conj": {
		...
		Schema: &test_utils.TableSchema{
			Columns: map[string]*Ydb.Type{
				"_id": testIdType,
				"a":   common.MakeOptionalType(common.MakePrimitiveType(Ydb.Type_INT32)),
				"b":   common.MakeOptionalType(common.MakePrimitiveType(Ydb.Type_STRING)),
				"ind": common.MakeOptionalType(common.MakePrimitiveType(Ydb.Type_INT32)),
			},
		},
		Records: []*test_utils.Record[[]byte, *array.BinaryBuilder]{{
	...
	"disj": {
		...
		Records: []*test_utils.Record[[]byte, *array.BinaryBuilder]{{
			Columns: map[string]any{
				...
				"b":   []*[]byte{ptr.Bytes([]byte("hello")), ptr.Bytes([]byte("hi")), ptr.Bytes([]byte("world"))},
	...
	"neg": {
		...
		Records: []*test_utils.Record[[]byte, *array.BinaryBuilder]{{
			Columns: map[string]any{
				...
				"boolean": []*uint8{ptr.Uint8(0)},
				"int64":   []*int64{nil},
				"string":  []*[]byte{ptr.Bytes([]byte("exists2"))},
				"objectid": []*[]byte{
					ptr.Bytes([]byte("507f1f77bcf86cd799439012")),
				},
			},
	...
	"bool": {
		...
		Records: []*test_utils.Record[[]byte, *array.BinaryBuilder]{{
			Columns: map[string]any{
				...
				"string":  []*[]byte{ptr.Bytes([]byte("inner"))},
	...
	"pushdown_in": {
		...
		Records: []*test_utils.Record[[]byte, *array.BinaryBuilder]{{
			Columns: map[string]any{
				"_id": [][]byte{[]byte("1"), []byte("3"), []byte("4")},
				"b":   []*[]byte{ptr.Bytes([]byte("four")), ptr.Bytes([]byte("two")), ptr.Bytes([]byte("hi"))},
	...
	"pushdown_regex": {
		...
		Records: []*test_utils.Record[[]byte, *array.BinaryBuilder]{{
			Columns: map[string]any{
				"_id": [][]byte{[]byte("0")},
				"a":   []*[]byte{ptr.Bytes([]byte("toast is great"))},
			},
		}},
	},
}
